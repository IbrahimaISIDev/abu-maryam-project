...
const i18n = {
  fr: {
    rewind: "Reculer 15 secondes",
    forward: "Avancer 30 secondes",
    pause: "Pause",
    play: "Lecture",
    close: "Fermer le player",
    mute: "Couper le son",
    unmute: "Rétablir le son",
    error: "Ce fichier audio est introuvable.",
  },
  ar: {
    rewind: "الرجوع 15 ثانية",
    forward: "التقدم 30 ثانية",
    pause: "إيقاف مؤقت",
    play: "تشغيل",
    close: "إغلاق المشغل",
    mute: "كتم الصوت",
    unmute: "إلغاء كتم الصوت",
    error: "هذا الملف الصوتي غير موجود.",
  },
};

export default function MiniPlayer() {
  const { state, pause, resume, close, seek, muted, toggleMute } = usePlayer();
  const { lang } = useDictionary();
  const t = i18n[lang];
  const { teaching, isPlaying, positionSeconds, hasError } = state;
  const progress = teaching ? (positionSeconds / teaching.durationSeconds) * 100 : 0;

  const minutes = teaching ? Math.floor(positionSeconds / 60) : 0;
  const seconds = teaching ? positionSeconds % 60 : 0;
  const timeStr = `${minutes}:${String(seconds).padStart(2, "0")}`;

  return (
    <div
      className={`fixed bottom-[64px] md:bottom-0 left-0 right-0 z-40 bg-[#fbf9f3] dark:bg-[#20261b] border-t border-[#e2dac9] dark:border-[#3a4132] shadow-[0_-2px_12px_rgba(0,0,0,0.08)] transition-transform duration-300 ease-out ${
        teaching ? "translate-y-0" : "translate-y-full pointer-events-none"
      }`}
    >
      ...
      {teaching && hasError ? (
        <div className="max-w-[1280px] mx-auto px-4 md:px-10 py-2.5 flex items-center gap-3">
          <span className="text-[#8a2f29] shrink-0" aria-hidden>⚠</span>
          <p className="flex-1 min-w-0 font-[var(--font-hanken)] text-[13px] text-[#8a2f29] line-clamp-1">
            {t.error}
          </p>
          <button type="button"
            onClick={close}
            className="w-8 h-8 flex items-center justify-center text-[#6f7363] dark:text-[#8f8973] hover:text-[#3c4a37] dark:hover:text-[#a9c19a] transition-colors shrink-0"
            aria-label={t.close}
          >
        ...
        <div className="max-w-[1280px] mx-auto px-4 md:px-10 py-2.5 flex items-center gap-3">
          ...
          <div className="w-9 h-9 rounded-full bg-[#eef0e6] dark:bg-[rgba(95,112,80,0.18)] flex items-center justify-center text-[#3c4a37] dark:text-[#a9c19a] text-[14px] shrink-0">
            {teaching ? typeIcon[teaching.type] : null}
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {/* Reculer 15s */}
            <button type="button"
              onClick={() => teaching && seek(Math.max(0, positionSeconds - 15))}
              className="hidden sm:flex w-8 h-8 items-center justify-center text-[#6f7363] dark:text-[#b7b2a0] hover:text-[#3c4a37] dark:hover:text-[#a9c19a] transition-colors"
              aria-label={t.rewind}
            >
            ...
            {/* Avancer 30s */}
            <button type="button"
              onClick={() => teaching && seek(Math.min(teaching.durationSeconds, positionSeconds + 30))}
              className="hidden sm:flex w-8 h-8 items-center justify-center text-[#6f7363] dark:text-[#b7b2a0] hover:text-[#3c4a37] dark:hover:text-[#a9c19a] transition-colors"
