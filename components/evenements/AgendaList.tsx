      ...
      <ul className="space-y-3">
        {filtered.map((item) => {
          ...
          return (
            <li
              key={item.id}
              className={`bg-[#fbf9f3] border rounded-[13px] flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-0 p-5 ${
                item.isFeatured ? "border-[#b58a3c]" : "border-[#e2dac9]"
              }`}
            >
              {/* Bloc date */}
              <div className="text-center shrink-0 sm:w-[80px]">
                <p className="font-[var(--font-hanken)] text-[11px] font-semibold uppercase text-[#9a9483]">
                  {d.month}
                </p>
                <p className="font-[var(--font-cormorant)] font-semibold text-[38px] text-[#232a20] leading-none">
                  {d.day}
                </p>
              ...
              {/* Séparateur vertical */}
              <div className="hidden sm:block w-px h-14 bg-[#e2dac9] mx-5 shrink-0" />

              {/* Texte */}
              <div className="flex-1 min-w-0">
                <p className="font-[var(--font-hanken)] text-[11.5px] font-semibold uppercase tracking-widest text-[#b58a3c] mb-1">
                  {typeLabel[item.type] ?? item.type}
                </p>
                <h3 className="font-[var(--font-cormorant)] font-semibold text-[22px] text-[#232a20] leading-tight">
                  {item.title}
                </h3>
                <p className="font-[var(--font-hanken)] text-[12.5px] text-[#9a9483] mt-1">
                  📍 {item.location}
                  ...
                  <Link
                    ...
                    className={`inline-block font-[var(--font-hanken)] font-semibold text-[13px] px-5 py-2.5 rounded-full transition-colors ${
                      item.isFeatured
                        ...
                        : "border border-[#d8d0bf] text-[#3f463a] hover:border-[#b58a3c] hover:text-[#b58a3c]"
                    }`}
                  >
                    {item.ctaLabel}
                  </Link>
